...
- data from user
    ...
    def __init__(self, algo, rig_hashrate, currency='USD', power_consumption=0,
        ...
        self.period = period
        self.usd_price = db.epic_vs_usd
        self.btc_price = db.epic_vs_btc
        self.rig_reward()
        self.rig_income()
        self.rig_cost()
        self.rig_profit()

    def find_algo(self, field):
        for x in db.blocks:
            if 'icemining' in x.values():
                if self.algo in x[field]:
                    # print('FOUND')
    ...
    def height(self):
        return int(self.model[0]['height'])

    def network_hash(self):
        data = self.find_algo('network_hashrate')
        return int(data[self.algo])

    def target_diff(self):
        data = self.find_algo('target_diff')
        return int(data[self.algo])

    def total_diff(self):
        data = self.find_algo('total_diff')
        return int(data[self.algo])
